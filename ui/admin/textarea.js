...
export const Textarea = function(el, opts) {
  ...
  */
  ...
  this.el.addEventListener('keydown', (ev) => {
    ...
    if (ev.which == 9) {
      ...
      dentSelection(this.el, ev.shiftKey, tab)
    }
  }, false)
}
...
Textarea.prototype.resizeToFit = function() {
  ...
  if (!isNaN(max_height) && shadow_height > max_height) {
    this.el.style.overflow = 'auto'
    this.el.style.height = ''
  }
  else if (!isNaN(min_height) && shadow_height < min_height) {
    this.el.style.overflow = 'auto'
    this.el.style.height = ''
  }
  else if (shadow_height != this.el.style.height) {
    // we are free to be flexible, just match the shadow!
    this.el.style.overflow = 'hidden'
    this.el.style.height = shadow_height
  }
}
